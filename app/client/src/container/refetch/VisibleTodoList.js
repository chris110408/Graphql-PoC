...
export default   ({currentUserId,activeVisibilityFilter,todos})=> {
    ...
    return (
        <TodoList
            ...
            actions={{
                completeTodo: (id,status) => completeTodo({ variables: { id ,status},refetchQueries: () => [{
                        query: GET_ALL_TODOS,   variables: { userId:currentUserId }
                    }]}),
                deleteTodo: (id) => deleteTodo({ variables: { id },refetchQueries: () => [{
                        query: GET_ALL_TODOS,   variables: { userId:currentUserId }
                    },{query: GET_ALL_Users}]}),
                editTodo: (id, text) => editTodo({ variables: { id, text },refetchQueries: () => [{
                        query: GET_ALL_TODOS,   variables: { userId:currentUserId }
                    }]}),
            }}/>
    )
